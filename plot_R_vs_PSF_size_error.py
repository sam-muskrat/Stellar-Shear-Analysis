...
y_boot = boot_df[[f'mean_R_{i}' for i in range(n_trials)]].to_numpy()  # (n_resamples, n_trials)


## ++++++++++++++ Workhorse: summarize + plot one x-variant (fwhm or T)

def summarize_and_plot(x, x_label, prefix, title_suffix):
    ...
    if args.paired or is_delta_lin or is_delta_quad:
        found = np.where(np.isclose(x, 0.0))[0]
        baseline_pos = found[0] if len(found) > 0 else None
        if baseline_pos is None:
            print(f">>> [{title_suffix}] No baseline (x=0) trial found - "
                  f"delta-fit/--paired residuals need one for this x-variant")
...
summarize_and_plot(x_fwhm, r'PSF FWHM Size Error ($\Delta$FWHM / FWHM$_{true}$)',
                    'fwhm', r'$\Delta FWHM / FWHM_{true}$')

if has_T:
    summarize_and_plot(x_T, r'$\Delta T / T_{true}$', 'T', r'$\Delta T / T_{true}$')
else:
