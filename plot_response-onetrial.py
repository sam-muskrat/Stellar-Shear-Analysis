...
parser.add_argument('-c', '--components', action='store_true',
                     help="Show the full response matrix (R11, R12, R21, R22) as 4 "
                          "histogram subplots instead of just the averaged R. Requires "
                          "R_per_cell.feather to have been generated by a version of "
                          "calculate_R_from_cata.py that computes the off-diagonal "
                          "terms (R12, R21).")
...
if args.components:
    ## Full response matrix (R11, R12, R21, R22) as 4 subplots, instead of
    ## just the averaged R below - requires calculate_R_from_cata.py to have
    ## computed the off-diagonal terms (R12, R21) when this file was made.
    required_cols = ['R11', 'R22', 'R12', 'R21']
    missing = [c for c in required_cols if c not in R_df.columns]
    if missing:
        print(f"Error: --components needs columns {required_cols}, but this file is "
              f"missing {missing} - rerun calculate_R_from_cata.py (now computes the "
              f"full response matrix) to regenerate R_per_cell.feather with them. "
              f"Aborting...")
        sys.exit(1)

    def plot_component(ax, values, weights, comp_label):
        mean_val = weighted_mean(values, weights)
        var_val = np.average((values - mean_val) ** 2, weights=weights)
        std_val = np.sqrt(var_val)

        res = bootstrap(
            (values, weights), weighted_mean, paired=True, vectorized=True,
            n_resamples=9999, method='basic', random_state=0,
        )
        se_val = res.standard_error

        ax.hist(values, bins=args.bins, edgecolor='black', alpha=0.7, weights=weights,
                label='Observed')
        ax.axvline(mean_val, color='red', linestyle='--', linewidth=2, alpha=0.7,
                   label=f'Mean = {mean_val:.6f}')
        ax.axvline(0, color='green', linestyle='--', linewidth=1, label='0 (expected for stars)')
        ax.axvspan(mean_val - se_val, mean_val + se_val, alpha=0.2, color='red',
                   label=f'±SE = {se_val:.6f}')
        ax.axvline(mean_val - se_val, color='orange', linestyle=':', linewidth=1.2, alpha=0.7)
        ax.axvline(mean_val + se_val, color='orange', linestyle=':', linewidth=1.2, alpha=0.7)

        ax.set_xlabel(f'Shear Response ({comp_label})', fontsize=11)
        ax.set_ylabel('Number of objects', fontsize=11)
        ax.set_title(comp_label, fontsize=12)
        ax.legend(fontsize=8)
        ax.grid(True, alpha=0.3)

        print(f"\n{comp_label} statistics:")
        print(f"  Mean (weighted): {mean_val:.6f}")
        print(f"  Std (weighted):  {std_val:.6f}")
        print(f"  Min:  {values.min():.6f}")
        print(f"  Max:  {values.max():.6f}")
        print(f"  Std. Err of Mean: {se_val:.6f}")
        print(f"  BCa 95% CI: {res.confidence_interval}")

    fig, axes = plt.subplots(2, 2, figsize=(14, 10))
    n_objects = R_df['n_objects'].to_numpy()
    for comp, ax in zip(['R11', 'R12', 'R21', 'R22'],
                         [axes[0, 0], axes[0, 1], axes[1, 0], axes[1, 1]]):
        plot_component(ax, R_df[comp].to_numpy(), n_objects, comp)

    fig.suptitle(args.title, fontsize=16)
    plt.tight_layout(rect=[0, 0, 1, 0.96])

    output_plot = 'shear_response_components.png'
    plt.savefig(output_plot, dpi=300, bbox_inches='tight')
    print(f"\nSaved component histograms to {output_plot}")
